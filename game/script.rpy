...
label yes:
    ...
    "Может Марту спросить? Или для нее это будет довольно личный вопрос, и она может не так это воспринять"
    menu ask:
...
label martha_good:
    ...
    "Марта взяла у повозчика ключи. Направилась к двери, чтобы открыть ее"
    stop music fadeout 1
    return
